    ...
    internal class AssemblyDescriptor : IAssemblyDescriptor
    {
        public AssemblyDescriptor(Assembly assembly)
        {
            ...
            if (assembly != null)
            {
                ...
                using (var resources = assembly.GetManifestResourceStream(ResourceConsts.UIResourceName))
                {
                    if (resources != null)
                    {
                        ...
                        var index = ResourcesIndexReaderWriter.Read(new SlicedStream(resources, 4, indexLength));
                        var baseOffset = indexLength + 4;
                        UIResources = index.ToDictionary(r => GetPathRooted(r), r => (IAssetDescriptor)
                            new UIResourceDescriptor(assembly, baseOffset + r.Offset, r.Size));
                    }
                }
            }
        }
