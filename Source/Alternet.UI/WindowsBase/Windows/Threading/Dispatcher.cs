...
using System.Runtime.InteropServices;        // SEHException
                           // SecurityCriticalData, TextServicesInterop
                   // WM
using Alternet.UI;               // SecurityHelper
...
namespace Alternet.UI.Threading
{
    ...
    public sealed class Dispatcher
    {
        ...
        static Dispatcher()
        {
            // yezo _msgProcessQueue = UnsafeNativeMethods.RegisterWindowMessage("DispatcherProcessQueue");
        ...
        */
        ...
        private Dispatcher()
        {
            ...
            // yezo MessageOnlyHwndWrapper window = new MessageOnlyHwndWrapper();
            // yezo _window = new SecurityCriticalData<MessageOnlyHwndWrapper>( window );

            // yezo _hook = new HwndWrapperHook(WndProcHook);
            // yezo _window.Value.AddHook(_hook);
        ...
        private void ShutdownImplInSecurityContext(Object state)
        {
            ...
            // yezo MessageOnlyHwndWrapper window = null;
            lock(_instanceLock)
            {
                // yezo window = _window.Value;
                // yezo _window = new SecurityCriticalData<MessageOnlyHwndWrapper>(null);
            }
            // yezo window.Dispose();
        ...
        private void PushFrameImpl(DispatcherFrame frame)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        // yezo if (!GetMessage(ref msg, IntPtr.Zero, 0, 0))
                        // yezo     break;

                        // yezo TranslateAndDispatchMessage(ref msg);
                    }
