...
namespace Alternet.UI
{
    public partial class AbstractControl
    {
        ...
        /// <summary>
        /// Called after item is inserted into the collection of children.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="index">Item index.</param>
        /// <param name="item">Item that was inserted.</param>
        protected virtual void OnChildrenItemInserted(
            object? sender,
            int index,
            AbstractControl item)
        {
            item.SetParentInternal(this);
            item.RaiseParentChanged(EventArgs.Empty);
            RaiseChildInserted(index, item);

            item.RaiseHandleCreated(EventArgs.Empty);
            ...
            void UpdateFontAndColor()
            {
                ...
                if (item.ParentBackColor)
                {
                    item.BackColor = RealBackgroundColor;
                }

                if (item.ParentForeColor)
                {
                    item.ForeColor = RealForegroundColor;
                }
            }
        }

        /// <summary>
        /// Called after item is removed from the collection of children.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="index">Item index.</param>
        /// <param name="item">Item that was removed.</param>
        protected virtual void OnChildrenItemRemoved(
            object? sender,
            int index,
            AbstractControl item)
        {
