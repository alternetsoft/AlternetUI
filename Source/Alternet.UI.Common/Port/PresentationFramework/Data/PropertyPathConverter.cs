            ...
            else
            {
                ...
                for (int i=0; i<originalPath.Length; ++i)
                {
                    ...
                    if (originalPath[i] == '(')
                    {
                        ...
                        int index;
                        if (Int32.TryParse( originalPath.AsSpan(i+1, j-i-1).ToString(),
                                            NumberStyles.Integer,
                                            TypeConverterHelper.InvariantEnglishUS.NumberFormat,
                                            out index))
                        {
                            // found (n). Write out the path so far, including the opening (
                            builder.Append(originalPath.AsSpan(start, i-start+1).ToString());
                ...
                {
                    builder.Append(originalPath.AsSpan(start).ToString());
                }
