...
namespace Alternet.UI.Integration.Remoting
{
    public abstract class TcpTransportBase
    {
        ...
        protected abstract IAlternetUIRemoteTransportConnection CreateTransport(IMessageTypeResolver resolver,
        ...
        public IDisposable Listen(IPAddress address, int port, Action<IAlternetUIRemoteTransportConnection> cb)
        {
        ...
        public async Task<IAlternetUIRemoteTransportConnection> Connect(IPAddress address, int port)
        {
            var c = new TcpClient();
