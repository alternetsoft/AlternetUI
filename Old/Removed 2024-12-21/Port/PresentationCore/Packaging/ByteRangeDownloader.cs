...
namespace Alternet.UI.Port
{
    ...
    internal class ByteRangeDownloader : IDisposable
    {
        ...
        internal ByteRangeDownloader(Uri requestedUri, string tempFileName, EventWaitHandle eventHandle)
        ...
        internal ByteRangeDownloader(Uri requestedUri, Stream tempStream, EventWaitHandle eventHandle, Mutex fileMutex)
            : this(requestedUri, eventHandle)
        {
        ...
        /// </summary>
        private ByteRangeDownloader(Uri requestedUri, EventWaitHandle eventHandle)
        {
        ...
        static private bool CheckContentRange(WebHeaderCollection responseHeaders, int beginOffset, ref int endOffset)
        {
            ...
            int firstByteOffset = Int32.Parse(contentRange.AsSpan(ByteRangeUnit.Length,
                                                                        index - ByteRangeUnit.Length).ToString(),
                                                NumberStyles.None, NumberFormatInfo.InvariantInfo);

            /*ReadOnlySpan<char>*/var contentRangeSpan = contentRange.AsSpan(index + 1).ToString();
            ...
            int lastByteOffset = Int32.Parse(contentRangeSpan.AsSpan().Slice(0, index).ToString(), NumberStyles.None, NumberFormatInfo.InvariantInfo);

            // Get the instance length
            // ContentRange: ZZZ
            contentRangeSpan = contentRangeSpan.AsSpan().Slice(index + 1).ToString();
        ...
        private EventWaitHandle _eventHandle;    // event handle which needs to be raised to inform the caller that
