...
namespace Alternet.UI.Port
{
    ...
    internal class BindingGroup : DependencyObject
    {
        ...
        internal override void AddInheritanceContext(DependencyObject context, DependencyProperty property)
        {
            ...
            // is amiss.
            if (/* yezo _hasMultipleInheritanceContexts && property != ItemsControl.ItemBindingGroupProperty && */TraceData.IsEnabled)
            {
                TraceData.TraceAndNotify(TraceEventType.Warning,
            ...
            if (mentor != null)
            {
                    ...
                    dataContextItem != BindingExpressionBase.DisconnectedItem)
                {
                    ...
                    if (itemReference == null ||
                        !PropertyPathWorker.EqualsEx(dataContextItem, itemReference.Target))
                    {
        ...
        static int FindIndexOf(object item, IList<WeakReference> list)
        {
            ...
            {
                if (PropertyPathWorker.EqualsEx(item, list[i].Target))
                {
                    return i;
                }
            }
                ...
                get
                {
                    for (int i=_table.Count-1; i >= 0; --i)
                    {
                        ...
                        if (propertyName == entry.PropertyName &&
                            PropertyPathWorker.EqualsEx(item, entry.Item))
                        {
                            return entry;
                        }
                    }
