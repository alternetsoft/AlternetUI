#!/bin/bash
set -euo pipefail

./BuildAndRun.CommonDialogsSample.sh
./BuildAndRun.ControlsSample.sh
./BuildAndRun.ControlsTest.sh
./BuildAndRun.CustomControlsSample.sh
./BuildAndRun.DataBindingSample.sh
./BuildAndRun.DragAndDropSample.sh
./BuildAndRun.DrawingSample.sh
./BuildAndRun.EmployeeFormSample.sh
./BuildAndRun.ExplorerUISample.sh
./BuildAndRun.HelloWorldSample.sh
./BuildAndRun.InputSample.sh
./BuildAndRun.LayoutSample.sh
./BuildAndRun.MenuSample.sh
./BuildAndRun.PaintSample.sh
./BuildAndRun.PrintingSample.sh
./BuildAndRun.ThreadingSample.sh
./BuildAndRun.WindowPropertiesSample.sh
